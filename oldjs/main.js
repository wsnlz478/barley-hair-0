/* ========================================
   ...
   ======================================== */
        ...
        /* ========================================================
           Embedded Contact Form Submitter
           - mirrors the exact submission logic of form2026.js
           - sends to the same backend: sendemail/send2026.php
           - adds mapping: EN <option values> -> CN whitelist required by send2026.php
           ======================================================== */
        (function () {
            var API = 'https://www.zhifazhifa.com/sendemail/send2026.php';
            // Embedded forms use EN short values; send2026.php only accepts these Simplified Chinese values.
            var PROJECT_MAP = {
                'hairline':   '发际线种植',
                'thickening': '头发加密',
                'baldness':   '秃顶植发',
            ...
            function showToast(text, isError) {
                var toast = $('embedded-form-toast') || Object.assign(document.createElement('div'), { id: 'embedded-form-toast' });
                Object.assign(toast.style, {
                    position: 'fixed', top: '50%', left: '50%', transform: 'translate(-50%,-50%)',
                    background: isError ? 'rgba(200,50,50,.92)' : 'rgba(30,80,162,.92)',
                    color: '#fff', padding: '14px 28px', borderRadius: '12px', fontSize: '15px',
                    zIndex: '999999', opacity: '0', transition: '.3s', pointerEvents: 'none',
            ...
            function initContactForm() {
                ...
                if (!btn || !form) return;
                if (btn.dataset.__embeddedFormInited === '1') return;
                btn.dataset.__embeddedFormInited = '1';

                btn.addEventListener('click', async function () {
                    var nickname = getVal('contactName');
                    var contact  = getVal('contactPhone');
                    var projectRaw = getVal('contactProject');
                    var message  = getVal('contactMessage');
                    // Map to CN whitelist; keep the raw value if a page already uses CN values (unlikely).
                    var project  = PROJECT_MAP[projectRaw] || projectRaw;

                    if (!nickname) { showToast('Please enter your name', true); return; }
                    if (!contact)  { showToast('Please enter your Phone / WeChat / WhatsApp / Email', true); return; }
                    if (!project || project === '') { showToast('Please select a service', true); return; }

                    var originalText = btn.textContent || btn.value || 'Send Message';
                    btn.disabled = true;
                    btn.textContent = 'Sending...';

                    var fd = new FormData();
                    fd.append('nickname',    safeText(nickname));
                    fd.append('project',     project);
                    fd.append('contact',     safeText(contact));
                    fd.append('message',     safeText(message));
                    fd.append('site_domain', window.location.hostname || 'www.hairimplant.cn');
                    fd.append('site_url',    window.location.href   || window.location.origin);

                    try {
                        var res  = await fetch(API, { method: 'POST', body: fd, cache: 'no-cache' });
                        var data = res.ok ? (await res.json().catch(function () { return {code:0,msg:'Server response error'}; }))
                                          : {code:0,msg:'Network error'};
                        if (data && (data.code === 1 || data.status === 'ok' || data.success === true)) {
                            showToast("Thank you! We'll contact you shortly within 24 hours.", false);
                            if (form && typeof form.reset === 'function') form.reset();
                        } else {
                            showToast((data && data.msg) ? data.msg : 'Submission failed, please try again', true);
                        }
                    } catch (e) {
